...
In a container without `/dev/uinput` the [Input Interposer](components/input-interposer.md) is used instead. It presents the pad only to applications started with it preloaded, which is why Steam and in-desktop browsers cannot find it there. With `publish_input_devices` on, off by default, a copy of the session's keyboard and pointer is published the same way, for applications that read evdev rather than the display server; not under host capture, where the session's own compositor already has them, and on an X server that reads kernel devices itself, a desktop host's Xorg, they stay on the interposer sockets, since a kernel device there would reach the session a second time.
...
**1. Run the [Desktop Container](components/desktop-image.md) (set `DISTRIB_FLAVOR` to an image flavor, `ubuntu26.04` or `debiantrixie`):**
...
python3 -c 'import os;from selkies.webrtc_ice_config import generate_rtc_config; print(generate_rtc_config(os.environ["SELKIES_TURN_HOST"], os.environ["SELKIES_TURN_PORT"], os.environ["SELKIES_TURN_SHARED_SECRET"], os.environ["SELKIES_TURN_USERNAME"]))'
```
