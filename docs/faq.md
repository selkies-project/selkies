...
In a container without `/dev/uinput` the [Input Interposer](component.md#input-interposer) is used instead. It presents the pad only to applications started with it preloaded, which is why Steam and in-desktop browsers cannot find it there.
