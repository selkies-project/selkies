...
A second display needs no configuration. XFCE and Plasma tile a maximized window across the whole framebuffer rather than the per-display regions, so this image asks Selkies to hand window management to Openbox the first time a session extends (`--multi-monitor-wm-swap`, off by default: restarting the window manager belongs to a session the deployment owns, not to a desktop somebody is using). A nested compositor cannot gain or lose a screen while it runs, so the session opens both at startup and Selkies holds the second at a token size until a client connects for it — the desktop stays the shape of what is being shown, and the screen grows to the second display's size the moment it is opened and shrinks back when it closes. `-e SELKIES_WAYLAND_OUTPUTS=N` fixes a different count.
