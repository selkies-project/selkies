...
- `build-pixelflux-pcmflux-wheels.yaml` puts the pixelflux and pcmflux built from their `main` HEAD into the run's artifacts, so images, packages, AppImages, and the suites ride the latest capture and audio code instead of the last PyPI release. Its locate job names each project's per-commit pre-release and a fetch job downloads it, one job per artifact the build would otherwise produce, so only a HEAD with none published yet is built here. Everything that needs the capture stack reads those artifacts, so a run resolves it once.
- `images.yaml` publishes the multi-architecture `base`, `example`, `coturn`, and `turn-rest` images to ghcr.io. Each architecture builds on its own native runner and pushes by digest; a merge job assembles the manifest, so no QEMU is involved. `base` and `example` are built in the same job, in that order, so the example layer names the base it was just given by digest rather than a tag that does not exist yet. A final job prunes the images no tag points at any more, which is what every push leaves behind when it moves the `main-*` tags onto new manifests.
...
- `release.yaml` is the maintainer entry point described below, and `release-published.yaml` applies its `latest` choice to the badge once the draft is published; `docs.yaml` publishes this site; `devcontainer-feature.yaml` validates and publishes the devcontainer feature.
...
- New releases are published by going to the [Release](https://github.com/selkies-project/selkies/actions/workflows/release.yaml) GitHub Action Workflow, and triggering `workflow_dispatch` by clicking on `Run workflow` with `Branch: main`, and specifying the release tag. The tag is a PEP 440 version without the leading `v`, either a release such as `1.2.3` or a pre-release such as `2.0.0rc0`; a pre-release is marked as one on the GitHub release. The `latest` input, on by default, designates the release as the latest everywhere: the floating `latest` image tags move onto it and the published release takes the "Latest" badge. Turn it off for a hotfix of an older series, or for a pre-release that should leave the image tags on the last full release; a pre-release never takes the badge, whichever way the input is set, because GitHub gives it to no pre-release. The draft release for the new proposed release will be generated in the [Releases](https://github.com/selkies-project/selkies/releases) page, only visible to the maintainers. After waiting for the release build to finish, editing the release notes, and publishing the release, the release will be visible as the latest release when the run designated it so; a run that did not leaves an invisible marker in the notes, which the `release-published.yaml` workflow reads at publication to keep the badge where it was. **If the same release is created multiple times because of certain issues, make sure to delete the previous release and the tag before running the [Release](https://github.com/selkies-project/selkies/actions/workflows/release.yaml) GitHub Action Workflow again.**
