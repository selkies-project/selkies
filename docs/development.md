...
- `build-pixelflux-pcmflux-wheels.yaml` puts the pixelflux and pcmflux built from their `main` HEAD into the run's artifacts, so images, packages, AppImages, and the suites ride the latest capture and audio code instead of the last PyPI release. Its locate job downloads each project's per-commit pre-release, and only a HEAD with none published yet is built here. Everything that needs the capture stack reads those artifacts, so a run resolves it once.
- `images.yaml` publishes the multi-architecture `example`, `coturn`, and `turn-rest` images to ghcr.io. Each architecture builds on its own native runner and pushes by digest; a merge job assembles the manifest, so no QEMU is involved. A final job prunes the images no tag points at any more, which is what every push leaves behind when it moves the `main-*` tags onto new manifests.
