...
| `SELKIES_FILE_TRANSFER_CC` | `--file-transfer-cc` | `True` | Pace file transfers so they do not queue ahead of the stream. Downloads adapt from the kernel's queue depth; uploads back off when the uploader's own session round trip inflates, taking only what the uplink has spare. Behind a reverse proxy the download gauge sees the hop to the proxy rather than the client's link, so use the static cap below there; the upload gauge times the client end to end and keeps working. |
