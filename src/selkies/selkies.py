...
# Backend switch from the settings pipeline (SELKIES_WAYLAND, legacy
# PIXELFLUX_WAYLAND alias, or --wayland). pixelflux never reads these itself:
...
try:
    ...
    # Mic-playback API (pcmflux 1.1.0+): the sole mic PCM data plane. hasattr-gated
    # so an older wheel degrades to a disabled microphone with a clear error.
    if hasattr(pcmflux, "AudioPlayback") and hasattr(pcmflux, "AudioPlaybackSettings"):
        AudioPlayback = pcmflux.AudioPlayback
        AudioPlaybackSettings = pcmflux.AudioPlaybackSettings
        ...
        data_logger.info("pcmflux library found. Audio capture is available (no mic playback).")
...
class DataStreamingServer(BaseStreamingService):
    ...
    async def _start_pcmflux_pipeline(self):
        ...
        try:
            ...
            # every connected client is capable. hasattr-guarded so a pcmflux
            # build predating the field can't crash startup.
            red_distance = self._compute_audio_red_distance()
            if hasattr(capture_settings, "red_distance"):
    ...
    async def _reconfigure_displays_locked(self):
        ...
        if self.capture_instances or any(d.get('backpressure_task') for d in self.display_clients.values()):
            # Serialize bulk teardown against per-display start/stop so an
            # in-flight START can't orphan a capture past this clear. Lock order:
            # _reconfigure_lock (already held) -> _video_capture_lock, same as the wrappers.
            async with self._video_capture_lock:
                ...
                if stop_bp_tasks:
                    await asyncio.gather(*stop_bp_tasks, return_exceptions=True)
                stop_capture_tasks = [
                    asyncio.to_thread(inst['module'].stop_capture)
    ...
    async def _start_capture_for_display_impl(self, display_id: str, width: int, height: int, x_offset: int, y_offset: int):
        ...
        if existing is not None:
            ...
            alive = True
            if module is not None and hasattr(module, 'is_capturing'):
                try:
            ...
            if alive:
                ...
                if module is not None and hasattr(module, 'request_idr_frame'):
                    try:
                        module.request_idr_frame()
                    except Exception:
                        pass
            ...
            if capture_module is None:
                if not X11_CAPTURE_AVAILABLE:
                    raise SelkiesAppError(
                        "ScreenCapture is not available because the pixelflux library is missing."
                    )
    ...
    def _get_capture_settings(self, display_id, width, height, x, y):
        ...
        # it frees the buffer once the last reference drops. Applies to H.264 on both
        # X11 and Wayland; only JPEG is excluded because its wire prefix re-framing
        # needs a fresh bytes copy.
        cs.deferred_free = encoder != "jpeg"
...
async def on_resize_handler(res_str, current_app_instance, data_server_instance=None, display_id='primary'):
    ...
    try:
        ...
        if data_server_instance and display_id in data_server_instance.display_clients:
            ...
            if IS_WAYLAND:
                logger_app_resize.info(f"Wayland Resize: Updating {display_id} to {target_w}x{target_h} and restarting pipeline.")
