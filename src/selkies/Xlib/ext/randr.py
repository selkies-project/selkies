...
# subpixel order - upstream note: these constants are part of the RENDER extension and
...
# Upstream note: this struct is part of the RENDER extension and belongs there
# if/when that extension is added to python-xlib.
Render_Transform = rq.Struct(
        rq.Card32('matrix11'), # Protocol note: all of these are listed as FIXED in the protocol header.
        rq.Card32('matrix12'),
        rq.Card32('matrix13'),
        rq.Card32('matrix21'),
        rq.Card32('matrix22'),
...
class GetScreenInfo(rq.ReplyRequest):
    ...
    _reply = rq.Struct(
        ...
        #rq.List('rates', RandR_Rates)  # disabled upstream: uncommenting causes a parse error
        )
...
class SetCrtcTransform(rq.Request):
    _request = rq.Struct(
        ...
        rq.List('filter_params', rq.Card32Obj), # Protocol note: the protocol says FIXED? http://cgit.freedesktop.org/xorg/proto/randrproto/tree/randrproto.txt#n2161
        )
...
class GetCrtcTransform(rq.ReplyRequest):
    ...
    _reply = rq.Struct(
        ...
        rq.List('pending_filter_params', rq.Card32Obj), # Protocol note: the protocol says FIXED? http://cgit.freedesktop.org/xorg/proto/randrproto/tree/randrproto.txt#n2161
        rq.String8('current_filter_name'),
        rq.List('current_filter_params', rq.Card32Obj), # Protocol note: the protocol says FIXED? http://cgit.freedesktop.org/xorg/proto/randrproto/tree/randrproto.txt#n2161
        )
