            ...
            try:
                if stype == "bool":
                    ...
                    if is_override:
                        ...
                        valid_items = [
                            item for item in user_items if item in master_list
                        ]
                        if not valid_items:
                            logging.warning(
                                f"Invalid value(s) '{raw_value}' for {name}. Using system default."
                            )
                        ...
                        if stype == "enum":
                            processed_value = (
                                valid_items[0] if valid_items else setting["default"]
                            )
                        else:  # list
