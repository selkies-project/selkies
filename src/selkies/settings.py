...
SETTING_DEFINITIONS_WEBRTC = [
    ...
    {'name': 'encoder', 'type': 'enum', 'default': 'x264enc', 'meta': {'allowed': ['x264enc', 'nvh264enc', 'vp8enc']}, 'help': 'GStreamer video encoder to use'},
    ...
    {'name': 'video_packetloss_percent', 'type': 'int', 'default': 0, 'help': 'Expected packet loss percentage (%) for ULP/RED Forward Error Correction (FEC) in video, use "0" to disable FEC, less effective because of other mechanisms including NACK/PLI, enabling not recommended if Google Congestion Control is enabled'},
    {'name': 'audio_channels', 'type': 'int', 'default': 2, 'help': 'Number of audio channels, defaults to stereo (2 channels)'},
    {'name': 'audio_packetloss_percent', 'type': 'int', 'default': 0, 'help': 'Expected packet loss percentage (%) for ULP/RED Forward Error Correction (FEC) in audio, use "0" to disable FEC'},
