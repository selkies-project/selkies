...
Output ids follow Selkies' mapping: the primary shows output 0 through the view
(id 1) covering it, and a secondary owns output 2.
...
def main() -> "H.Results":
    ...
    created = capture.create_output(2, 1920, 1080, 1920, 0, 1.0)
    outputs = sorted(o[0] for o in capture.list_outputs())
    res.check("second output created beside the primary's screen and view",
              created and outputs == [0, 1, 2], outputs)
    ...
    try:
        ...
        res.check("a new output takes the waiting screen",
                  capture.create_output(2, 1920, 1080, 1920, 0, 1.0)
