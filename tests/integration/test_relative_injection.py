...
edge must not leave a stale tracked position behind that swallows the next
absolute move.
"""
import asyncio
import json
...
def x11_block(res) -> None:
    ...
    try:
        ...
        pinned = pointer_x()
        res.check("x11: an edge-pinned drag leaves no stale position behind",
                  pinned == 200, f"{pinned}")
    finally:
        d.close()
        H.server_stop()
...
def wayland_block(res) -> None:
    ...
    try:
        ...
        seen = [line["x"] for line in obs.lines
                if line.get("kind") in ("ptr_enter", "ptr_motion")]
        pinned = seen[-1] if seen else float("nan")
        res.check("wayland: an edge-pinned drag leaves no stale position behind",
                  pinned == 200, f"{pinned}")
    finally:
        obs.stop()
        H.server_stop()
