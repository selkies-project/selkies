...
frame is the only repair. The checks live in tests/tools/color_range_audit.mjs,
...
AUDIT = os.path.join(TESTS, "tools", "color_range_audit.mjs")

node = shutil.which("node")
if not node:
    # Reported as a skip, never as a pass: the audit is the whole suite, so
    # exiting 0 here would announce the space is right without having looked.
    print("SKIP node not found, so the color range audit cannot run", flush=True)
...
if not lines:
    print(f"FAIL  [color-range] audit ran  {r.stderr.strip()[:400]}", flush=True)
    sys.exit(1)

passed = sum(1 for ln in lines if ln.startswith("PASS"))
print(f"[color-range] {passed}/{len(lines)} passed")
