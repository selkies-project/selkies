...
export class WebRTCDemo {
    ...
    _onPeerDataChannelMessage(event) {
        ...
        if (msg.type === 'pipeline') {
            ...
            // msg-end indicates the end of chucks of a clipboard msg
            if (msg.type === "clipboard-msg-end") {
                ...
                this.clipboardcontent.length = 0; // Clear the clipboard content after processing
            } else {
    ...
    createAuxDataChannel() {
        if (this._aux_channel === null) {
            this._aux_channel = this.peerConnection.createDataChannel("data", {
                // Data channels defaults to 'ordered' and not 'negotiated'; explicitly mentioned for readability
                ordered: true,      
                negotiated: false, 
            });
            this._aux_channel.onopen = () => {
                this._setStatus("An auxiliary data channel opened up");
                this._aux_channel.bufferedAmountLowThreshold = 16777216  // 16MiB as the threshold
            }
            this._aux_channel.onmessage = (msg) => {
                this._setDebug("Received msg from aux data channel: ", msg);
            }
            this._aux_channel.onerror = () => {
                this._aux_channel = null;
                this._setError("Auxiliary data channel error");
            }
            this._aux_channel.onclose = () => {
                this._aux_channel = null;
                this._setStatus("Gracefully closed the aux data channel");
            }
            return true;
        }
        return false;
    ...
    sendAuxChannelData(data) {
        if (this._aux_channel !== null && this._aux_channel.readyState === 'open') {
            ...
            console.warn("Aux data channel closed or uninitialized")
        }
    }
    ...
    isAuxBufferNearThreshold() {
        if (this._aux_channel === null || this._aux_channel.readyState !== 'open') {
            console.warn("Aux data channel does not exist");
            return false;
        } else {
            let threshold = this._aux_channel.bufferedAmount / this._aux_channel.bufferedAmountLowThreshold;
            if (threshold >= 0.9) {
                this._setDebug(`Aux data channel buffer reached ${Math.trunc(threshold*100)}% threshold`);
                return true;
            }
        }
        return false;
     ...
     */
    awaitForAuxBufferToDrain() {
        return new Promise(resolve => {
            // If buffer is already empty, resolve immediately
            if (this._aux_channel.bufferedAmount === 0) {
                resolve();
                return;
            }
